    ...
    def __getitem__(self, index):
        ...
        if self.cfg.NO_TRANS==False:
            if 'seg' == self.cfg.TARGET_MODAL:
                seg = (color_label_np(label_copy, ignore=self.ignore_label,dataset='cityscapes').astype(np.uint8))
...
class ADE20K(torch.utils.data.Dataset):
    def __init__(self,cfg=None,transform=None,phase_train=True,data_dir=None):
        ...
        if phase_train:
            ...
            trainFlag="validation/"
        self.img_dir = data_dir + "images/"+trainFlag
        self.label_dir = data_dir + "annotations/"+trainFlag
        self.examples = []
        ...
        for file_name in file_names:
            ...
            label_path = self.label_dir+img_id+'.png' 
            ...
            # example["label"] = label_copy



            # example["seg"] = seg_path_npy
            self.examples.append(example)
        self.num_examples = len(self.examples)
    def __len__(self):
        return self.num_examples
    def __getitem__(self, index):
        ...
        if self.cfg.NO_TRANS==False:
            if 'seg' == self.cfg.TARGET_MODAL:
                seg = (color_label_np(label, ignore=self.ignore_label,dataset=ade20k).astype(np.uint8))
                seg=cv2.cvtColor(np.asarray(seg), cv2.COLOR_RGB2BGR)
                # seg=np.load(seg_path)
    ...
    def __call__(self, sample):
        ...
        for key in sample.keys():
            if key == 'label':
                ...
                continue
            sample[key] = cv2.resize(sample[key], None, fx=scale_factor_x, fy=scale_factor_y, interpolation=cv2.INTER_LINEAR)
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        for key in sample.keys():
            sample[key] = sample[key][h_off:h_off+self.crop_h, w_off:w_off+self.crop_w]
...
class Normalize(transforms.Normalize):
    ...
    def __init__(self, mean, std, ms_targets=[]):
        ...
        self.ms_targets = ms_targets
        self.mean = [item *255 for item in mean]
        self.std = [item *255 for item in std]
    ...
    def __call__(self, sample):
        ...
        for key in single_targets:
            ...
            for t, m, s in zip(sample['image'], self.mean, self.std):
                t.sub_(m).div_(s)
            if 'seg' in sample.keys():
                for t, m, s in zip(sample['seg'], self.mean, self.std):
                    t.sub_(m).div_(s)
        return sample